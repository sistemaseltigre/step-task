...
async function createUnstake(
  ...
  const stakeProcess = await program.methods
    ...
    .unstake(vaultBump, new anchor.BN(anchorTokenAmount))
    .accounts({
      tokenMint: STEP_MINT,
      xTokenMint: XSTEP_MINT,
      xTokenFrom: xstepAddress,
...
export default function UnstakeToken({ tokenAmount }: { tokenAmount: String }) {
  ...
  useEffect(() => {
    if (connected && publicKey && sendTransaction) {
      ...
      createUnstake(connection, publicKey, sendTransaction, wallet, tokenAmount)
        .then(() => {
